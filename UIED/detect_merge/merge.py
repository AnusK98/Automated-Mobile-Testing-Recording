...
def show_elements(org_img, eles, show=False, win_name='element', wait_key=0, shown_resize=None, line=2, remove_id=[]):
    ...
    for i, ele in enumerate(eles):
        ...
        # if there is children in the element
        # if len(ele.children) <= 0:
        ele.visualize_number(img, counter = i, back_color = color)
    img_resize = img
...
def refine_elements(compos, texts, intersection_bias=(2, 2), containment_ratio=0.1):
    '''
...
def remove_bottom_bar(elements, img_height):
    ...
    for ele in elements:
        ...
        if ele.row_min > (img_height - img_height/10):
            continue
        new_elements.append(ele)
    return new_elements
...
def merge(img_path, compo_path, text_path, merge_root=None, is_paragraph=False, is_remove_bar=True, show=False, wait_key=0, remove_id = []):
    ...
    if is_paragraph:
        elements = merge_text_line_to_paragraph(elements, max_line_gap=1)
    reassign_ids(elements)
    check_containment(elements)
    board = show_elements(img_resize, elements, show=show, win_name='elements after merging', wait_key=wait_key, remove_id=remove_id)
