...
class Element:
    ...
    def calc_distance(self, element_b):
        a = self.put_bbox()
        b = element_b.put_bbox()
        # Calculate the shortest distance between the four points of the bounding box
        distances = []
        for point_a in [(a[0], a[1]), (a[0], a[3]), (a[2], a[1]), (a[2], a[3])]:
            for point_b in [(b[0], b[1]), (b[0], b[3]), (b[2], b[1]), (b[2], b[3])]:
    ...
    def visualize_number(self, img, font_scale=0.6, thickness=1, show=False, counter = 1, back_color = (0, 0, 0)):
        loc = self.put_bbox()
        # Calculate the center of the bounding box
        center_x = loc[0] + (loc[2] - loc[0]) // 3
        center_y = loc[1] + (loc[3] - loc[1]) // 3

        # Convert the counter number to string
        text = str(counter)
        
        # Get the text size to align it in the center
        text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, thickness)[0]
        
        # Calculate text position so that the text is centered
        ...
        avg_color = np.mean(img[rect_top:rect_bottom, rect_left:rect_right], axis=(0, 1))
        ...
        # text_color = (255-back_color[0], 255-back_color[1], 255-back_color[2])
